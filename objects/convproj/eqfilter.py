...
class cvpj_eq:
	def __init__(self, plugin_obj, basename):
		...
		self.filtnum = 0
		self.plugstate_obj = plugin_obj
		self.num_bands = 0
		self.bands = []

	def __iter__(self):
		for x in self.bands:
			yield x, self.plugstate_obj.named_filter[x] if x in self.plugstate_obj.named_filter else cvpj_filter()
	...
	def to_8limited(self, convproj_obj, pluginid):
		...
		ftypes = [self.plugstate_obj.named_filter[f].type.type for f in fnames]
		...
		if ftypes.count('high_pass')<2 and ftypes.count('low_shelf')<2 and ftypes.count('peak')<5 and ftypes.count('high_shelf')<2 and ftypes.count('low_pass')<2:
			...
			for num in range(len(fnames)):
				fname = fnames[num]
				ftype = self.plugstate_obj.named_filter[fname].type.type
