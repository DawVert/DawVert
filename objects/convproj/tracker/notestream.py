...
class notestream:
	def __init__(self, assoc_instid):
		...
		self.assoc_instid = assoc_instid
		self.cur_pos = 0
		self.note_pos = 0
		self.slide_speed = 0
		self.vol = 1
	...
	def note_on(self, c_note, c_fx):
		...
		if self.cur_inst != None:
			last_nl.add_m(self.cur_inst, self.cur_pos, 0, c_note, self.vol, {'pan': init_pan} if init_pan else None)
		self.slide_pitch = notelist_auto.pitchmod(c_note)
	...
	def do_cell(self, c_note, c_inst, c_fx, s_speed):
		...
		if c_inst != None: 
			self.cur_inst = self.assoc_instid[c_inst] if c_inst in self.assoc_instid else 'unknown_'+str(c_inst)
			self.vol = 1
