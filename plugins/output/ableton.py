...
def do_warpmarkers(convproj_obj, WarpMarkers, stretch_obj, sampleref_obj, dur_sec, pitch):
	...
	aftersec = warpmarker_obj.BeatTime = dsr if stretch_obj.timing.tempo_based else dsr/(2**(pitch/12))
	...
	return warpenabled, 1 if stretch_obj.timing.tempo_based else 1/(2**(pitch/12))
...
def do_audioclips(convproj_obj, pls_audio, track_color, als_track):
	for clipid, audiopl_obj in enumerate(pls_audio):
		...
		if s_timing_obj.time_type == 'warp' and warp_obj.points:
			...
			for clipid, notespl_obj in enumerate(track_obj.placements.pl_notes):
				...
				for key, notedata in t_keydata.items():
					...
					for t_id, t_pos, t_dur, t_vol, t_inst, t_extra, t_autopack in notedata:
						...
						if t_extra:
							if 'off_vol' in t_extra: MidiNoteEvent_obj.OffVelocity = t_extra['off_vol']*127
