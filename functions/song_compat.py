...
class song_compat:
	...
	def process_part(self, process_name, classname, convproj_obj, cvpj_type, in_compat, out_compat, out_type):
		if process_name not in self.finished_processes:
			if classname.process(convproj_obj, in_compat, out_compat, out_type):
				logger_compat.info(process_name+' Done.')
				self.finished_processes.append(process_name)

	def makecompat(self, convproj_obj, cvpj_type, in_dawinfo, out_dawinfo, out_type):
		...
		if self.currenttime == False:
			...
			self.process_part('sep_nest_audio', sep_nest_audio,	   convproj_obj, cvpj_type, in_dawinfo.audio_nested, out_dawinfo.audio_nested, out_type)
			self.process_part('changestretch', changestretch,		 convproj_obj, cvpj_type, in_dawinfo.audio_stretch, out_dawinfo.audio_stretch, out_type)
			self.process_part('removecut', removecut,				 convproj_obj, cvpj_type, in_dawinfo.placement_cut, out_dawinfo.placement_cut, out_type)
			self.process_part('loops_add', loops_add,				 convproj_obj, cvpj_type, in_dawinfo.placement_loop, out_dawinfo.placement_loop, out_type)
			self.process_part('nopl_track', nopl_track,			   convproj_obj, cvpj_type, in_dawinfo.track_nopl, out_dawinfo.track_nopl, out_type)
